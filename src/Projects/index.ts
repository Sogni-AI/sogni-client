...
/** Outcome of looking a project up on the REST API once the socket no longer lists it. */
...
class ProjectsApi extends ApiGroup<ProjectApiEvents> {
  ...
  /**
   * Look up projects the socket no longer lists. The REST API stores a project
   * only once it finishes and the socket posts it asynchronously, so a 404 is
   * retried a few times before the project is declared lost.
   *
  ...
  private async _processImageAssets(project: Project, data: ImageProjectParams) {
    ...
    // Context images (GPT Image 2 supports up to 16; Qwen Image Edit supports up to 3; Krea 2 Identity Edit and legacy Flux Kontext support up to 2)
