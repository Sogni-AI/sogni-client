// Hosted-tool argument validator. Pure JSON-Schema-ish logic with no
// external dependencies. Lifted from @sogni-ai/sogni-intelligence-client's
// src/contracts/hostedToolValidation.ts to break the sogni-client <->
// intelligence-client circular dependency. Both copies should stay in
// behavioural lockstep; consider a CI diff-check if you change either.
//
// Features:
//   - JSON-Schema subset (type, const, enum, bounds, length, pattern,
//     items, properties, required, additionalProperties)
//   - Pattern auto-anchoring (`pattern: 'abc'` matches the whole string)
...
function validateStringConstraints(
  ...
  if (schema.minLength !== undefined && value.length < schema.minLength) {
    errors.push(
      `Argument "${path}" must contain at least ${schema.minLength} character${schema.minLength === 1 ? '' : 's'}`
    );
  }
  if (schema.maxLength !== undefined && value.length > schema.maxLength) {
    errors.push(
      `Argument "${path}" must contain at most ${schema.maxLength} character${schema.maxLength === 1 ? '' : 's'}`
    );
  }
  if (schema.pattern) {
    try {
...
function validateArrayAgainstSchema(
  ...
  if (schema.minItems !== undefined && value.length < schema.minItems) {
    context.errors.push(
      `Argument "${path}" must contain at least ${schema.minItems} item${schema.minItems === 1 ? '' : 's'}`
    );
  }
  if (schema.maxItems !== undefined && value.length > schema.maxItems) {
    context.errors.push(
      `Argument "${path}" must contain at most ${schema.maxItems} item${schema.maxItems === 1 ? '' : 's'}`
    );
  }
  ...
  return value.map(
    (item, index) =>
      validateValueAgainstSchema(
        `${path}[${index}]`,
        item,
        schema.items as HostedToolSchemaProperty,
        context
      ).value
  );
}
