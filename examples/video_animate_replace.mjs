/**
 * Animate-Replace Example using WAN 2.2
 *
 ...
 *   node video_animate_replace.mjs --prompt "A cartoon character with vibrant colors"
 *   node video_animate_replace.mjs --model wan_v2.2-14b-fp8_animate-replace_lightx2v
 *   node video_animate_replace.mjs --steps 6
 *   node video_animate_replace.mjs --seconds 5 --fps 32
 *   node video_animate_replace.mjs --width 640 --height 480
 *
 * Options:
 *   --prompt <text>   Custom prompt for the replacement (optional, has default)
 ...
 *   --seconds <n>     Video duration in seconds (default: 5, max: 20s @16fps or 10s @32fps)
...
const DEFAULT_POSITIVE_PROMPT = 'Seamless subject replacement, natural motion preserved, high quality';
const DEFAULT_NEGATIVE_PROMPT = 'blurry, low quality, distorted, artifacts, watermark, text, flickering';

// Reference assets for animate-replace workflow
const REFERENCE_IMAGE = './test-assets/placeholder.jpg'; // New subject
const REFERENCE_VIDEO = './test-assets/placeholder.mp4'; // Video with subject to replace
...
async function askSpeedOrQuality() {
  ...
  if (!process.stdin.isTTY) {
    return MODELS['animate-replace'].speed;
  }
  ...
  return new Promise((resolve) => {
    rl.question('Enter choice [1/2] (default: 1): ', (answer) => {
      ...
      if (choice === '2' || choice.toLowerCase() === 'quality' || choice.toLowerCase() === 'q') {
        ...
        resolve(MODELS['animate-replace'].quality);
      } else {
        console.log('  → Using Speed mode\n');
        resolve(MODELS['animate-replace'].speed);
      }
    });
  });
}
...
async function main() {
  ...
  console.log('Sogni Animate-Replace (via Wan 2.2 14B)');
  console.log('='.repeat(60));
  console.log();
  ...
  const referenceImagePath = await pickImageFile(null, 'reference image (new subject)');
  const referenceVideoPath = await pickVideoFile(null, 'reference video (motion source)');

  console.log(`✓ Reference image (new subject): ${referenceImagePath}`);
  console.log(`✓ Reference video (motion source): ${referenceVideoPath}`);
  console.log();
