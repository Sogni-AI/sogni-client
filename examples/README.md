...
**Upscale an image up to 16K (RTX VSR):**
...
Enlarge an existing image up to a 15360px longest edge with NVIDIA RTX Video
...
**Output bounds:** the longest output edge is capped at 15360px, every output
edge must be at least 512px, dimensions are aligned down to multiples of 8,
and aspect ratio is always preserved. Sources already at 15360px on their
longest edge cannot be upscaled further.

Note: with neither `--scale` nor `--target`, the script targets the 15360px
maximum, which is a large and correspondingly expensive job. Pass `--target`
or `--scale` to cap it; interactive runs show a cost estimate and confirm
before spending.
...
- **Generation** — `generate_image` (defaults to `krea2_turbo_fp8_scaled`), `edit_image`, `generate_video` (defaults to `ltx23-22b-fp8_t2v_distilled`), `generate_music` (defaults to `ace_step_1.5_xl_turbo`), `sound_to_video`, `video_to_video`
...
Guided mode defaults text-to-video to `/v1/creative-agent/workflows` so the entrypoint exercises the durable workflow API first. Scripted T2V calls without media still default to `/v1/chat/completions` unless `--workflow` is passed. Media modes default to `/v1/creative-agent/workflows` with explicit `input.steps` and upload local media from `test-assets` automatically. Pass `--image`/`--context`, `--audio`, or `--video` repeatedly to use Seedance multimodal context; Seedance 2.5 permits 30 images, 10 videos, 10 audios, and 50 files total, while the 2.0 family retains 9/3/3/12. Use Seedance-style role tags in prompts (`@Image1`, `@Video1`, `@Audio1`) counted independently by modality in attachment order, and prefer positive preservation instructions. `--expand-prompt` is enabled by default and sends `expand_prompt: true` so the API runs the shared `@sogni/creative-agent` Seedance LLM prompt shaper before dispatch; pass `--no-expand-prompt` only when you want to submit the compact prompt directly. `--no-execute` prints the workflow request without submitting it; local media is still uploaded first so the printed request contains real HTTPS media URLs. Seedance is billed by the vendor per render, so scripted runs that submit need `--confirm-vendor-spend` or a yes at the terminal (the guided picker's proceed prompt counts); `workflow_seedance_2_5_r2v.mjs` and `workflow_creative_agent_workflows.mjs` apply the same rule to any vendor model (GPT Image, Seedance, HappyHorse, Wan 3). Use `--no-estimate` when you only want to inspect request construction.

For the Seedance 2.5 reference/edit/extend contract specifically, use the non-billable dry-run example. Its direct SDK layer sends `seedanceTaskType`; its Creative Agent layer maps the same intent to semantic workflow tools without leaking that vendor routing field:

```bash
node workflow_seedance_2_5_r2v.mjs "Use @Audio1 to guide a new performance" --task-type reference --audio https://cdn.example.com/voice.mp3 --dry-run
node workflow_seedance_2_5_r2v.mjs "Edit @Video1 while preserving its subject and timing" --task-type edit --video https://cdn.example.com/source.mp4 --duration 5 --dry-run
node workflow_seedance_2_5_r2v.mjs "Extend @Video1 after its ending" --task-type extend --video https://cdn.example.com/source.mp4 --duration 8 --dry-run
node workflow_seedance_2_5_r2v.mjs "Edit @Video1" --task-type edit --video https://cdn.example.com/source.mp4 --creative-agent --dry-run
```

The mappings are `reference` → `generate_video`, `edit` → `video_to_video`, and `extend` → `extend_video`. Seedance 2.5 uses 480p/720p at 24fps for 4-30 seconds. `edit` and `extend` require `@Video1`; direct edit should use the source clip's duration, while extend uses the requested continuation duration. The 2.0 family omits `seedanceTaskType`, stays at 4-15 seconds and 9/3/3/12 references, and does not accept audio-only context.
